...
if page == "Home":
    ...
    info_cols = st.columns([0.1, 0.8, 0.1])
    with info_cols[1]:
        ...
        st.bar_chart(chart_data, x='Outlet Type', y='Total Sales (in Millions ₹)', use_container_width=True)
        st.markdown("`Supermarket Type1` outlets account for the majority of sales, followed by `Supermarket Type3`. `Grocery Stores` have a much smaller market share.")
        st.markdown("---")

        st.subheader("Sales Distribution by Location")
        st.markdown("Visualizations like these are commonly built in BI tools like Power BI to create interactive dashboards for stakeholders.")
        ...
        fig, ax = plt.subplots()
        # Use a color scheme that works well in both light and dark themes
        colors = ['#FF9900', '#1CADE4', '#5CB85C']
        ax.pie(pie_data['Sales'], labels=pie_data['Location Type'], autopct='%1.1f%%', startangle=90, colors=colors,
               wedgeprops=dict(width=0.4, edgecolor='w' if st.session_state.theme == 'dark' else 'k'))
        ax.axis('equal')  # Equal aspect ratio ensures that pie is drawn as a circle.
        fig.set_facecolor('#F7F7F7' if st.session_state.theme == 'dark' else '#232F3E') # Match background
        text_color = 'black' if st.session_state.theme == 'dark' else 'white'
        plt.setp(ax.texts, size=10, color=text_color)

        st.pyplot(fig)
        st.image("https://i.imgur.com/vTU4y2b.png", caption="Example of a professional Power BI sales dashboard.")
    ...
    with info_cols[1]:
        ...
        st.markdown("<div class='centered-content'>- **Item Fat Content:** The fat content category of the product (e.g., Low Fat, Regular).<br>- **Item Type:** The specific category the product belongs to (e.g., Dairy, Snack Foods).<br>- **Item MRP (₹):** The Maximum Retail Price of the product in Indian Rupees.<br>- **Item Visibility:** The percentage of display area in a store allocated to this product.<br>- **Item Weight (kg):** The weight of the product in kilograms.<br>- **Outlet Identifier:** A unique ID for the store where the product is sold.<br>- **Outlet Size:** The relative size of the store (Small, Medium, or High).<br>- **Outlet Location Type:** The tier of the city where the store is located (Tier 1, 2, or 3).<br>- **Outlet Type:** The format of the retail outlet (e.g., Supermarket, Grocery Store).<br>- **Outlet Age (Years):** The number of years the outlet has been operational.</div>", unsafe_allow_html=True)
